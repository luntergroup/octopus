...
BOOST_AUTO_TEST_CASE(can_copy_haplotype_subregions)
{
    BOOST_REQUIRE(test_file_exists(human_reference_fasta));
    ...
    const auto copy_region = parse_region("16:9299964-9300083", human);
    const auto copied = copy<Haplotype>(haplotype, copy_region);
    
    BOOST_CHECK(is_same_region(copied, copy_region));
    BOOST_CHECK(contains(haplotype, copied));
    BOOST_CHECK(copy<Haplotype>(haplotype, region) == haplotype);
    
    const auto allele_region = encompassing_region(alleles);
    const auto allele_region_copy = copy<Haplotype>(haplotype, allele_region);
    
    BOOST_CHECK(std::all_of(std::cbegin(alleles), std::cend(alleles),
                            [&] (const auto& allele) { return allele_region_copy.includes(allele); }));
    
    const auto partial_allele_region = parse_region("16:9299947-9300074", human);
    const auto partial_allele_region_copy = copy<Haplotype>(haplotype, partial_allele_region);
    
    BOOST_CHECK(!partial_allele_region_copy.contains(allele1));
    BOOST_CHECK(!partial_allele_region_copy.includes(allele2));
    BOOST_CHECK(!partial_allele_region_copy.contains(allele13));
    ...
    const auto insertion_region_haplotype_copy = copy<Haplotype>(haplotype, insertion_region);
    
    BOOST_CHECK(is_same_region(insertion_region_haplotype_copy, insertion_region));
    BOOST_CHECK(insertion_region_haplotype_copy.sequence() == "");
    BOOST_CHECK(insertion_region_haplotype_copy.includes(allele5));
    
    const auto insertion_region_allele_copy = copy<Allele>(haplotype, insertion_region);
    BOOST_CHECK(insertion_region_allele_copy == allele5);
    const auto insertion_before_snp_region = head_region(allele9);
    const auto insertion_before_snp_haplotype_copy = copy<Haplotype>(haplotype, insertion_before_snp_region);
    
    BOOST_CHECK(is_same_region(insertion_before_snp_haplotype_copy, insertion_before_snp_region));
    BOOST_CHECK(insertion_before_snp_haplotype_copy.sequence() == "");
    BOOST_CHECK(is_reference(insertion_before_snp_haplotype_copy));
    
    const auto insertion_before_snp_allele_copy = copy<Allele>(haplotype, insertion_before_snp_region);
    
    BOOST_CHECK(is_empty_region(insertion_before_snp_allele_copy));
    BOOST_CHECK(is_reference(insertion_before_snp_allele_copy, human));
}

BOOST_AUTO_TEST_CASE(copying_around_insertions_works)
{
    ...
    auto copy_hap_ref0 = copy<Haplotype>(ref_haplotype, mapped_region(variant0));
    auto copy_hap_alt0 = copy<Haplotype>(alt_haplotype, mapped_region(variant0));
    
    BOOST_CHECK(is_same_region(copy_hap_ref0, variant0));
    BOOST_CHECK(copy_hap_ref0.sequence() == "");
    BOOST_CHECK(is_same_region(copy_hap_alt0, variant0));
    BOOST_CHECK(copy_hap_alt0.sequence() == "A");
    BOOST_CHECK(copy_hap_ref0.difference(copy_hap_alt0).size() == 1);
    ...
    auto copy_hap_ref4 = copy<Haplotype>(ref_haplotype, mapped_region(variant4));
    auto copy_hap_alt4 = copy<Haplotype>(alt_haplotype, mapped_region(variant4));
    
    BOOST_CHECK(is_same_region(copy_hap_ref4, variant4));
    BOOST_CHECK(copy_hap_ref4.sequence() == "");
    BOOST_CHECK(is_same_region(copy_hap_alt4, variant4));
    BOOST_CHECK(copy_hap_alt4.sequence() == "A");
    BOOST_CHECK(copy_hap_ref4.difference(copy_hap_alt4).size() == 1);
    ...
    auto copy_hap_ref6 = copy<Haplotype>(ref_haplotype, mapped_region(variant6));
    auto copy_hap_alt6 = copy<Haplotype>(alt_haplotype, mapped_region(variant6));
    
    BOOST_CHECK(is_same_region(copy_hap_ref6, variant6));
    BOOST_CHECK(copy_hap_ref6.sequence() == "");
    BOOST_CHECK(is_same_region(copy_hap_alt6, variant6));
    BOOST_CHECK(copy_hap_alt6.sequence() == "A");
    BOOST_CHECK(copy_hap_ref6.difference(copy_hap_alt6).size() == 1);
