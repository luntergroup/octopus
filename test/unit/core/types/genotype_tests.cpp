...
BOOST_AUTO_TEST_SUITE(genotype)
...
BOOST_AUTO_TEST_CASE(extract_all_elements_works_correctly)
{
    ...
    auto haplotypes = generate_all_haplotypes(variants, human);
...
BOOST_AUTO_TEST_CASE(copy_works_correctly)
{
    BOOST_REQUIRE(test_file_exists(human_reference_fasta));
    ...
    const auto allele_copy_snp1 = copy<Allele>(genotype, mapped_region(variant1));
    BOOST_CHECK(!allele_copy_snp1.is_homozygous());
    BOOST_CHECK(allele_copy_snp1.contains(variant1.ref_allele()));
    BOOST_CHECK(allele_copy_snp1.contains(variant1.alt_allele()));

    const auto allele_copy_snp2 = copy<Allele>(genotype, mapped_region(variant2));
    BOOST_CHECK(!allele_copy_snp2.is_homozygous());
    BOOST_CHECK(allele_copy_snp2.contains(variant2.ref_allele()));
    BOOST_CHECK(allele_copy_snp2.contains(variant2.alt_allele()));

    const auto allele_copy_snp3 = copy<Allele>(genotype, mapped_region(variant3));
    BOOST_CHECK(!allele_copy_snp3.is_homozygous());
    BOOST_CHECK(allele_copy_snp3.contains(variant3.ref_allele()));
    BOOST_CHECK(allele_copy_snp3.contains(variant3.alt_allele()));

    const auto allele_copy_snp4 = copy<Allele>(genotype, mapped_region(variant5));
    BOOST_CHECK(!allele_copy_snp4.is_homozygous());
    BOOST_CHECK(allele_copy_snp4.contains(variant5.ref_allele()));
    BOOST_CHECK(allele_copy_snp4.contains(variant5.alt_allele()));

    const auto allele_copy_insertion = copy<Allele>(genotype, mapped_region(variant4));
    BOOST_CHECK(!allele_copy_insertion.is_homozygous());
    BOOST_CHECK(allele_copy_insertion.contains(variant4.ref_allele()));
    BOOST_CHECK(allele_copy_insertion.contains(variant4.alt_allele()));
}

BOOST_AUTO_TEST_CASE(copy_all_works_correctly)
{
    BOOST_REQUIRE(test_file_exists(human_reference_fasta));
    ...
    const auto variant4 = make_variant("6:31235414-31235415", "A", human);
    const std::vector<Variant> variants {variant1, variant2, variant3, variant4};
    const auto haplotypes = generate_all_haplotypes(variants, human);
    const auto genotypes  = generate_all_genotypes(haplotypes, 2);
    const auto allele_copies = copy_all<Allele>(genotypes, mapped_region(variant2));
    BOOST_REQUIRE(allele_copies.size() == 4);
    const auto haplotype_copies1 = copy_all<Haplotype>(genotypes, mapped_region(variant2));
    BOOST_REQUIRE(haplotype_copies1.size() == num_genotypes(2, 2));
    const auto haplotype_copies2 = copy_all<Haplotype>(genotypes, encompassing_region(variant2, variant3));
    BOOST_REQUIRE(haplotype_copies2.size() == num_genotypes(4, 2));
}
