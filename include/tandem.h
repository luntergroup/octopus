...
namespace detail
{
    ...
    uint32_t forward_lce(const T& str, uint32_t i, uint32_t j, uint32_t n)
    {
        return static_cast<uint32_t>(
            std::distance(std::cbegin(str) + i, std::mismatch(std::cbegin(str) + i, std::cbegin(str) + n, std::cbegin(str) + j).first)
        );
    }
    ...
    {
        return static_cast<uint32_t>(
            std::distance(std::crend(str) - i - 1, std::mismatch(std::crend(str) - i - 1, std::crend(str) - n, std::crend(str) - j - 1).first)
        );
    }
...
namespace detail
{
        ...
        for (uint32_t k {}; k < lz_blocks.size(); ++k) {
            ...
            for (auto j = block.pos; j < block_end; ++j) {
                ...
                if (num_targets > 0) {
                    ...
                    // std::vector beats std::deque here as we only need to do this front
                    // insertion once for each position
    ...
    template <typename T>
    std::size_t get_num_runs(const std::vector<T>& buckets)
    {
...
find_maximal_repetitions(const T& str, const uint32_t min_period = 1, const uint32_t max_period = -1)
{
    ...
    for (auto& bucket : sorted_buckets) {
        result.insert(result.end(), std::cbegin(bucket), std::cend(bucket));
        bucket.clear();
        bucket.shrink_to_fit();
    }
