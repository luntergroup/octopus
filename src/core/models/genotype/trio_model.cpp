...
namespace octopus
{
    namespace model
    {
        ...
        TrioModel::TrioModel(const Trio &trio,
            ...
            : trio_{trio}, prior_model_{prior_model}, mutation_model_{mutation_model}, options_{options}, debug_log_{debug_log}
        {
        }
        ...
        namespace
        {
                                    ...
                                    const TrioModel::GenotypeVector &genotypes)
            {
                ...
                std::transform(std::cbegin(likelihoods), std::cend(likelihoods), std::begin(posteriors),
                               [&](const auto &p)
                               { return p.probability + prior_model.evaluate({genotypes[p.genotype]}); });
            ...
            {
                template <typename T>
                const auto &operator()(const T &x) const noexcept { return x.probability; }
            };
            ...
            auto compute_min_joint(const std::vector<T> &zipped, const double max_log_mass_loss, boost::optional<double> &lost_log_mass)
            {
                ...
                const static auto log_sum_exp = [](auto sum, auto x)
                { return maths::log_sum_exp(sum, x); };
            ...
            {
                auto zipped = index_zip(first, last, [](const auto &pair)
                ...
                std::transform(std::begin(zipped), kth, std::begin(result), [](const auto &p)
                               { return p.second; });
                return result;
            }
            ...
            {
                return std::any_of(first_parent, last_parent, [&](const auto &parent)
                                   { return is_represented(haplotype, parent, genotypes); });
            }
            ...
            auto find_represented(const Haplotype &haplotype,
                                  ...
                                  const TrioGenotypeData &genotypes)
            {
                return std::find_if(first_parent, last_parent, [&](const auto &parent)
                                    { return is_represented(haplotype, parent, genotypes); });
            }
            ...
            {
                if (!options.max_genotype_combinations)
                    return make_reduction_map(parents, std::cend(parents), options);
                        ...
                        const TrioModel::GenotypeVector &genotypes)
            {
                ...
                else
                {
                    ...
                    const auto is_reference_genotype = [&](const auto &p)
                    { return is_homozygous_reference(genotypes[p.genotype]); };
            ...
            {
                return mother.probability + father.probability + joint_probability(genotypes.maternal[mother.genotype], genotypes.paternal[father.genotype], model);
            }
            ...
            double probability_of_child_given_diploid_parents(const H &child_from_mother, const H &child_from_father,
                                                              ...
                                                              const DeNovoModel &mutation_model)
            {
                return probability_of_child_given_diploid_parent(child_from_mother, mother, mutation_model) + probability_of_child_given_diploid_parent(child_from_father, father, mutation_model);
            }
            ...
            double probability_of_child_given_triploid_parents(const H &child1_from_mother,
                                                               ...
                                                               const DeNovoModel &mutation_model)
            {
                return probability_of_child_given_triploid_parent(child1_from_mother, mother, mutation_model) + probability_of_child_given_triploid_parent(child2_from_mother, mother, mutation_model) + probability_of_child_given_triploid_parent(child_from_father, father, mutation_model);
            }
            ...
            template <>
            struct ProbabilityOfChildGivenParents<2, 2, 2>
            {
                ProbabilityOfChildGivenParents(const DeNovoModel &mutation_model) : mutation_model{mutation_model} {}
                template <typename G>
                double operator()(const G &child, const G &mother, const G &father)
            ...
            template <>
            struct ProbabilityOfChildGivenParents<3, 3, 3>
            {
                ProbabilityOfChildGivenParents(const DeNovoModel &mutation_model) : mutation_model{mutation_model} {}
                template <typename G>
                double operator()(const G &child, const G &mother, const G &father)
            ...
            template <>
            struct ProbabilityOfChildGivenParents<2, 2, 1>
            {
                ProbabilityOfChildGivenParents(const DeNovoModel &mutation_model) : mutation_model{mutation_model} {}
                template <typename G>
                double operator()(const G &child, const G &mother, const G &father)
            ...
            template <>
            struct ProbabilityOfChildGivenParents<1, 2, 1>
            {
                ProbabilityOfChildGivenParents(const DeNovoModel &mutation_model) : mutation_model{mutation_model} {}
                template <typename G>
                double operator()(const G &child, const G &mother, const G &father)
            ...
            template <>
            struct ProbabilityOfChildGivenParents<1, 0, 1>
            {
                ProbabilityOfChildGivenParents(const DeNovoModel &mutation_model) : mutation_model{mutation_model} {}
                template <typename G>
                double operator()(const G &child, const G &mother, const G &father)
            ...
            template <>
            struct ProbabilityOfChildGivenParents<1, 1, 1>
            {
                ProbabilityOfChildGivenParents(const DeNovoModel &mutation_model) : mutation_model{mutation_model} {}
                template <typename G>
                double operator()(const G &child, const G &mother, const G &father)
            ...
            {
                return parents.probability + child.probability + joint_probability_function(genotypes.child[child.genotype], genotypes.maternal[parents.maternal], genotypes.paternal[parents.paternal]);
            }
            ...
            auto extract_probabilities(const std::vector<JointProbability> &joint_likelihoods)
            {
                ...
                std::transform(std::cbegin(joint_likelihoods), std::cend(joint_likelihoods),
                               std::begin(result), [](const auto &p)
                               { return p.log_probability; });
                return result;
            }
                            ...
                            const HaplotypeLikelihoodArray &haplotype_likelihoods) const
        {
            ...
            if (debug_log_)
                debug::print(stream(*debug_log_), genotypes, parental_likelihoods);
            ...
            if (lost_log_mass)
                *lost_log_mass *= 2 * std::distance(reduced_child_likelihoods_info.first, reduced_child_likelihoods_info.last_full_join);
        ...
        auto joint_probability(const GenotypeIndexProbabilityPair &parent,
                               ...
                               const DeNovoModel &mutation_model)
        {
            return parent.probability + child.probability + probability_of_child_given_parent(child_genotypes[child.genotype], parent_genotypes[parent.genotype], mutation_model);
        }
                                     ...
                                     const HaplotypeLikelihoodArray &haplotype_likelihoods) const
        {
            ...
            if (debug_log_)
                debug::print(stream(*debug_log_), "child", child_genotypes, child_likelihoods);
            boost::optional<double> lost_log_mass{};
            ...
            if (debug_log_)
                debug::print(stream(*debug_log_), "parent", child_genotypes, parent_likelihoods);
            ...
            if (lost_log_mass)
                *lost_log_mass *= 2 * std::distance(reduced_child_likelihoods.first, reduced_child_likelihoods.last_full_join);
