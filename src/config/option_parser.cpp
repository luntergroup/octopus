...
OptionMap parse_options(const int argc, const char** argv)
{
    ...
    ("filter-expression",
     po::value<std::string>()->default_value("QUAL < 10 | MQ < 10 | MP < 10 | AFB > 0.25 | SB > 0.98 | BQ < 15 | DP < 1"),
    ...
    ("denovo-filter-expression",
     po::value<std::string>()->default_value("QUAL < 50 | PP < 40 | GQ < 20 | MQ < 30 | AFB > 0.2 | SB > 0.95 | BQ < 20 | DP < 10 | DC > 1 | MF > 0.2 | FRF > 0.5 | MP < 30 | MQ0 > 2"),
     "Boolean expression to use to filter somatic variant calls")
