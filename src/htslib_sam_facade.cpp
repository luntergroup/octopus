                                        ...
                                        size_t max_coverage)
{
    ...
    if (samples_.size() > 1) {
        while (sample != sample_names_.at(it.get_read_group())) {
            if (!++it) {
                return std::make_pair(head_region(region), std::vector<unsigned> {});
            }
        };
    ...
    if (samples_.size() == 1) {
        ...
        while (++it && sample == sample_names_.at(it.get_read_group()) && --max_coverage > 0) {
            read_begin = it.get_begin();
