...
: associate_ {std::move(associate)}, type_ {std::move(type)} {}

MissingIndexError::MissingIndexError(Path associate, Path given_index, std::string type)
: associate_ {std::move(associate)}, given_index_ {std::move(given_index)}, type_ {std::move(type)} {}
...
std::string MissingIndexError::do_why() const
{
    ...
    if (given_index_) {
        ss << "The index file you specified " << *given_index_ << " for the " << type_ << " file "
